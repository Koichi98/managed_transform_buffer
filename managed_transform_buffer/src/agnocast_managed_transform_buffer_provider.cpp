...
#include "managed_transform_buffer/agnocast_managed_transform_buffer_provider.hpp"
...
AgnocastManagedTransformBufferProvider & AgnocastManagedTransformBufferProvider::getInstance(
  rcl_clock_type_t clock_type, const bool force_dynamic, tf2::Duration discovery_timeout,
  tf2::Duration cache_time)
{
  static AgnocastManagedTransformBufferProvider instance(clock_type, discovery_timeout, cache_time);
  if (force_dynamic) {
...
std::optional<TransformStamped> AgnocastManagedTransformBufferProvider::getTransform(
...
bool AgnocastManagedTransformBufferProvider::isStatic() const
{
  return is_static_.load();
}

rclcpp::Clock::SharedPtr AgnocastManagedTransformBufferProvider::getClock() const
{
  return clock_;
}

AgnocastManagedTransformBufferProvider::AgnocastManagedTransformBufferProvider(
  ...
  logger_(rclcpp::get_logger("agnocast_managed_transform_buffer"))
{
  ...
  tf_buffer_ = std::make_unique<agnocast::Buffer>(clock_, cache_time);
...
void AgnocastManagedTransformBufferProvider::activateListener()
{
  ...
  node_ = std::make_shared<agnocast::Node>("_", options_);
  ...
  using callback_t =
    std::function<void(agnocast::ipc_shared_ptr<tf2_msgs::msg::TFMessage> &&)>;
  callback_t cb = [this](agnocast::ipc_shared_ptr<tf2_msgs::msg::TFMessage> && msg) {
    tfCallback(std::move(msg), false);
  };
  callback_t cb_static = [this](agnocast::ipc_shared_ptr<tf2_msgs::msg::TFMessage> && msg) {
    tfCallback(std::move(msg), true);
  };
...
void AgnocastManagedTransformBufferProvider::deactivateListener()
{
  ...
  // agnocast executor does not support remove_callback_group, so cancel and recreate
  executor_->cancel();
  if (executor_thread_->joinable()) {
    executor_thread_->join();
  }
...
std::string AgnocastManagedTransformBufferProvider::generateUniqueNodeName()
{
...
void AgnocastManagedTransformBufferProvider::tfCallback(
  agnocast::ipc_shared_ptr<tf2_msgs::msg::TFMessage> && msg, const bool is_static)
{
  const tf2_msgs::msg::TFMessage & msg_in = *msg;
  std::string authority = "Authority undetectable";
  for (const auto & tf : msg_in.transforms) {
    try {
...
std::optional<TransformStamped> AgnocastManagedTransformBufferProvider::lookupTransform(
  const std::string & target_frame, const std::string & source_frame, const tf2::TimePoint & time,
...
TraverseResult AgnocastManagedTransformBufferProvider::traverseTree(
...
std::optional<TransformStamped> AgnocastManagedTransformBufferProvider::getDynamicTransform(
  const std::string & target_frame, const std::string & source_frame, const tf2::TimePoint & time,
  const tf2::Duration & timeout, const rclcpp::Logger & logger) const
{
...
std::optional<TransformStamped> AgnocastManagedTransformBufferProvider::getStaticTransform(
  const std::string & target_frame, const std::string & source_frame, const tf2::TimePoint & time)
{
...
std::optional<TransformStamped> AgnocastManagedTransformBufferProvider::getUnknownTransform(
  const std::string & target_frame, const std::string & source_frame, const tf2::TimePoint & time,
  const tf2::Duration & timeout, const rclcpp::Logger & logger)
{
