...
#ifndef MANAGED_TRANSFORM_BUFFER__AGNOCAST_MANAGED_TRANSFORM_BUFFER_HPP_
#define MANAGED_TRANSFORM_BUFFER__AGNOCAST_MANAGED_TRANSFORM_BUFFER_HPP_
...
constexpr tf2::Duration AGNOCAST_DISCOVERY_TIMEOUT = std::chrono::milliseconds(20);

/**
 * @brief A managed TF buffer for agnocast::Node that handles listener node lifetime.
 * This buffer triggers listener only for first occurrence of frames pair.
 * After that, the local buffer is used for storing static transforms.
 * If a dynamic transform is detected, the listener is switched to dynamic mode
 * and acts as a regular TF buffer.
 */
class AgnocastManagedTransformBuffer
{
public:
  /**
   ...
   */
  explicit AgnocastManagedTransformBuffer(
    rcl_clock_type_t clock_type = RCL_ROS_TIME, const bool force_dynamic = false,
    tf2::Duration discovery_timeout =
      tf2::Duration(managed_transform_buffer::AGNOCAST_DISCOVERY_TIMEOUT),
    tf2::Duration cache_time = tf2::Duration(tf2::BUFFER_CORE_DEFAULT_CACHE_TIME));

  /** @brief Destroy the Agnocast Managed Transform Buffer object */
  ~AgnocastManagedTransformBuffer() = default;
...
private:
  ...
  AgnocastManagedTransformBufferProvider * provider_;
};

}  // namespace managed_transform_buffer

#endif  // MANAGED_TRANSFORM_BUFFER__AGNOCAST_MANAGED_TRANSFORM_BUFFER_HPP_
